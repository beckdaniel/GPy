...
#m.constrain_fixed('rbf2_var',1.) # Variance parameter will be given by the elements of the coregionalization matrix
m.constrain_positive('kappa')
#m.constrain_positive('W')
m.constrain_fixed('iip',m.Z[:,m.input_cols].flatten()) #No need to optimize this
m.set('len',.2) #NOTE the model works better initializing lengthscale as .1
#m.set('2_len',.1) #NOTE the model works better initializing lengthscale as .1
#m.set('rbf_._var', 0)
m.set('W',np.random.normal(0,1,15))
m.set('kappa',1.)
# Optimize
